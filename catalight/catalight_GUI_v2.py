...
from catalight.equipment.eqpt_thread import Eqpt_Controller
...
from PyQt5.QtCore import (QObject, QRunnable, Qt, QThreadPool, QTimer,
                          pyqtSignal, pyqtSlot, QThread, QMetaObject,
                          Q_ARG)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.eqpt_controller.moveToThread(eqpt_thread)
        QMetaObject.invokeMethod(self.eqpt_controller, "init_equipment")
        self.eqpt_controller.connection_progress.\
            connect(self.loading_screen.progress_bar.setValue)
        self.eqpt_controller.connection_status.\
            connect(self.loading_screen.status_msg.setText)

        self.eqpt_controller.initialized.connect(self.init_app)
        self.eqpt_controller.eqpt_status_signal.connect(self.update_eqpt_status)
        ...
        self.emergencyStop.clicked.connect(self.eqpt_controller.emergency_stop)
    ...
    def init_app(self):
        ...
        # TODO clean this up:
        # is gc_Status.setChecked() a slot?
        # self.gc_Status.setChecked.connect(self.eqpt_thread.gc_status)
        # self.gas_Status.setChecked.connect(self.eqpt_thread.gas_status)
        # self.heater_Status.setChecked.connect(self.eqpt_thread.heater_status)
        # self.laser_Status.setChecked.connect(self.eqpt_thread.laser_status)
    ...
    def start_study(self):
        ...
        # # TODO this may be missing an arg
        QMetaObject.invokeMethod(self.eqpt_controller, "begin_study",
                                 Qt.QueuedConnection,
                                 Q_ARG(object, expt_list))
        # TODO this should wait for study to finish.
    ...
    def select_ctrl_file(self):
        ...
        if self.gc_Status.isChecked():
            ...
            QMetaObject.invokeMethod(self.eqpt_controller, "load_ctrl_file",
                                     Qt.QueuedConnection,
                                     value0=Q_ARG(str, filePath))
    ...
    def select_cal_file(self):
        ...
        self.cal_path.setText(filePath)  # Update display in GUI
        calDF = pd.read_csv(filePath, delimiter=',', index_col='Chem ID')
        if self.gas_Status.isChecked():
            # Add CalGas to hardware settings
            QMetaObject.invokeMethod(self.eqpt_controller, 'load_cal_file',
                                     Qt.QueuedConnection,
                                     value0=Q_ARG(object, calDF))
            # Add option for CalGas to GUI elements
    ...
    def init_manual_ctrl_tab(self, eqpt_status):
        """
        ...
        if eqpt_status['gas connected']:
            flow_dict = eqpt_status['flows']
        ...
        if eqpt_status['heater connected']:  # Initialize Values for Heater
            self.manualTemp.setValue(eqpt_status['temp_setpoint'])
            self.manualRamp.setValue(eqpt_status['heater_ramp_rate'])

        if eqpt_status['diode connected']:  # Initialize Values for Laser
    ...
    def update_eqpt_status(self, eqpt_status):
        """
        ...
        """
        if eqpt_status['diode connected']:
            # Status
        ...
        if eqpt_status['heater connected']:
            # Status
            self.heater_Status.setChecked(1)
            self.current_temp_1.setText('%.2f' % eqpt_status['temp'])
            self.current_temp_2.setText('%.2f' % eqpt_status['temp'])
            self.current_temp_setpoint1.setText('%.2f' % eqpt_status['temp_setpoint'])
            self.current_temp_setpoint2.setText('%.2f' % eqpt_status['temp_setpoint'])
    ...
    def closeEvent(self, *args, **kwargs):
        ...
        #TODO call stuff from the other thread here
