...
class Diode_Laser():
    ...
    def set_power(self, P_set):
        ...
        The necessary current is sent based on a externally performed calibration.
        Outputs read power, set point, and time to console.
        ...
        # Vout_value = ul.from_eng_units(self.board_num, self._ao_range, Vout)  # Convert to 16bit
    
        # ul.a_out(self.board_num, 0, self._ao_range, Vout_value)  # Send signal to DAQ Board
        # Vin_value = ul.a_in(self.board_num, self.channel, self._ai_range)
        # Vin_eng_units_value = ul.to_eng_units(self.board_num, self._ai_range, Vin_value)
    
        # print('Reading = ' + str(Vin_eng_units_value*self._k_mod))
        print('Set Point = ' + str(P_set))
        print(time.ctime())
        volume_control.SetMasterVolumeLevel(-2.0, None) # Sets Vol in dB -0.0 is 100%
    ...
    def update_calibration(self, slope, intercept):
        ...
        old_cal_file = open(calibration_path,'r+')
        new_cal_file = []
        
        for line in old_cal_file: # read in the values after '=' sign line by line
            if re.search('m = ', line):
                line = ('m = ' + str(slope) + ' \n')
    ...
    def read_calibration(self):
        ...
        calibration = open(calibration_path,'r')

        for line in calibration: # read in the values after '=' sign line by line
            if re.search('m = ', line):
                self._calibration[0] = float(line.split('=')[-1].strip(' \n'))
            elif re.search('b = ', line):
                self._calibration[1] = float(line.split('=')[-1].strip(' \n'))
            elif re.search('date = ', line):
                print('Last laser calibration was:')
                print(line)
        print(f'Power = {self._calibration[0]}*Current + {self._calibration[1]}')
        
        calibration.close()
        
    def time_warning(self, time_left):
        '''Enter time in minutes until activation of laser, read outs warning message'''
        # Consider upgrading this to use asyncio or threading.Timer and have the
        # code put out 5 4 3 2 1 minute warnings on a seperate thread
        volume_control.SetMasterVolumeLevel(-2.0, None) # Sets Vol in dB -0.0 is 100%
        voice_control.say(
